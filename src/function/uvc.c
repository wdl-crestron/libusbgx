...
const char * format_names[MAX_FORMATS] = {
    ...
    UVC_PATH_STREAMING_UNCOMPRESSED_FRAMEBASED,
};
...
struct {
    ...
    [USBG_F_UVC_CONFIG_STREAMING_INTERVAL] = UVC_DEC_ATTR(streaming_interval),
    [USBG_F_UVC_CONFIG_FUNCTION_NAME] = UVC_STRING_ATTR(function_name),
};

#undef UVC_DEC_ATTR

static int frame_intervals_get(const char *path, const char *name, const char *attr, void *val)
{
    struct usbg_f_uvc_frame_intervals *desc = val;
    int ret;

    ret = usbg_read_decs(path, name, attr, desc->value, ARRAY_SIZE(desc->value));
...
static int frame_intervals_set(const char *path, const char *name, const char *attr, const void *val)
{
    const struct usbg_f_uvc_frame_intervals *desc = val;

    if (desc->len == 0) return USBG_SUCCESS;

    int ret = usbg_write_decs(path, name, attr, desc->value, desc->len);
...
#define UVC_FRAME_INTERVALS_ATTR(_name)                      \
    {                               \
        .name = #_name,                     \
        .offset = offsetof(struct usbg_f_uvc_frame_attrs, _name),     \
        .get = frame_intervals_get,                        \
        .set = frame_intervals_set,                        \
    }
...
struct {
    ...
    [USBG_F_UVC_FRAME_INTERVALS] = UVC_FRAME_INTERVALS_ATTR(intervals),
    [USBG_F_UVC_FRAME_HEIGHT] = UVC_DEC_ATTR(wHeight),
    [USBG_F_UVC_FRAME_WIDTH] = UVC_DEC_ATTR(wWidth),
};

#undef UVC_DEC_ATTR
#undef UVC_FRAME_INTERVALS_ATTR
...
static int guid_set(const char *path, const char *name, const char *attr, const void *val)
{
    ...
    int ret = usbg_write_buf(path, name, attr, desc->value, desc->len);
...
static int uvc_set_frame(char *format_path, const char *format, const struct usbg_f_uvc_frame_attrs *attrs)
{
    ...
    ret = usbg_write_decs(frame_path, frame_name, "dwFrameInterval", attrs->intervals.value, attrs->intervals.len);
    if (ret != USBG_SUCCESS)
        return ret;
